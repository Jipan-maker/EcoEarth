...
1.  **Clone Repository**
    ```bash
    git clone https://github.com/Jipan-maker/EcoEarth.git
    cd EcoEarth
    ```
...
3.  **Jalankan Aplikasi**
    Buka terminal dan jalankan notebook atau script utama (sesuaikan dengan cara run project Anda):
    ```bash
    # Contoh jika menggunakan script python
    python app.py
    ```
    Atau buka `EcoEarth.ipynb` di Jupyter Notebook/VS Code.
